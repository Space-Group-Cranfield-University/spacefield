...
% assuming J2 effect. In particular, the script plots the coverage
% increase as a function of the number of satellites for three different
...
for k = 1:3
    ...
    coverageTemp = [];
    tripleCoverageTemp = [];
    coverageIncreaseTemp = [];
    tripleCoverageIncreaseTemp = [];
    ...
    for j = 0:satStep:nSat
        if k == 1
            ConstellationParameters.nSatOrb = ConstellationParameters.nSatOrb + satStep;
            Constellation = initialiseConstellation(ConstellationParameters, Constants);
        elseif k == 2
            Constellation = [];
            for kConstellation = 1:2
                ConstellationParametersTemp = ConstellationParameters;
                ConstellationParametersTemp.nSatOrb = ConstellationParametersTemp.nSatOrb + floor(j/2) + (kConstellation==1) * mod(j, 2);
                ConstellationTemp = initialiseConstellation(ConstellationParametersTemp, Constants);
                startSat = (kConstellation-1)*ConstellationParametersTemp.nSatOrb +1;
                finishSat = kConstellation*ConstellationParametersTemp.nSatOrb;
                Constellation = [Constellation ConstellationTemp(startSat:finishSat)];
            end
        else
            Constellation = [];
            for kConstellation = 1:5
                ConstellationParametersTemp = ConstellationParameters;
                ConstellationParametersTemp.nSatOrb = ConstellationParametersTemp.nSatOrb + floor(j/5) + ((mod(j,5) - kConstellation)>=0);
                ConstellationTemp = initialiseConstellation(ConstellationParametersTemp, Constants);
                startSat = (kConstellation-1)*ConstellationParametersTemp.nSatOrb +1;
                finishSat = kConstellation*ConstellationParametersTemp.nSatOrb;
                Constellation = [Constellation ConstellationTemp(startSat:finishSat)];
        ...
        [coverageVecTemp, tripleCoverageVecTemp]     = mainCoverage(sample, dirSun, R, ...
                        Constellation, VisibilityParams);
        coverageTemp = [coverageTemp; mean(coverageVecTemp)];
        tripleCoverageTemp = [tripleCoverageTemp; mean(tripleCoverageVecTemp)];
        if j > 0
            coverageIncreaseTemp = [coverageIncreaseTemp; (coverageTemp(end)-coverageTemp(end-1))/coverageTemp(end-1)];
            tripleCoverageIncreaseTemp = [tripleCoverageIncreaseTemp; (tripleCoverageTemp(end)-tripleCoverageTemp(end-1))/tripleCoverageTemp(end-1)];
        end
        disp("Coverage computed, time: "+toc+", satellites: "+j+" / "+nSat)
    end
    disp("Constellation completed, time: "+toc)
    coverageVec = [coverageVec coverageTemp];
    tripleCoverageVec = [tripleCoverageVec tripleCoverageTemp];
    coverageIncrease = [coverageIncrease coverageIncreaseTemp];
    tripleCoverageIncrease = [tripleCoverageIncrease tripleCoverageIncreaseTemp];
end
...
hold on
for k = 1:3
    stairs(nSatTot + [0 satVec], coverageVec(:, k)*100, "LineWidth", 2)
end
fontsize(gca, 10, 'points')
ylabel("single coverage [%]", "FontSize", 12)
xlabel("number of satellites", "FontSize", 12)
subplot(2,1,2)
hold on
for k = 1:3
    scatter(nSatTot + satVec, coverageIncrease(:, k)*100, "filled", "LineWidth", 2)
end
lgd = legend(legendConstellationType);
lgd.Location = "southwest";
fontsize(gca, 10, 'points')
ylabel("single cov. increase [%]", "FontSize", 12)
xlabel("number of satellites", "FontSize", 12)
...
hold on
for k = 1:3
    stairs(nSatTot + [0 satVec], tripleCoverageVec(:, k)*100, "LineWidth", 2)
end
fontsize(gca, 10, 'points')
...
hold on
for k = 1:3
    scatter(nSatTot + satVec, tripleCoverageIncrease(:, k)*100, "filled", "LineWidth", 2)
end
lgd = legend(legendConstellationType);
lgd.Location = "southwest";
fontsize(gca, 10, 'points')
ylabel("triple cov. increase [%]", "FontSize", 12)
xlabel("number of satellites", "FontSize", 12)

%% Functions

function [coverageVec, tripleCoverageVec]     = mainCoverage(sample, dirSun, R, ...
                Constellation, VisibilityParameters)  
    % 3) Compute coverage at all timesteps

...
% assuming J2 effect. The complete script takes approximately 100 s to run
% for a single constellation. In particular, the script plots coverage
% variation over one orbital period for three different constellation
% geometries.
...
%SizeData{:, 1} = SizeData{:,1}/2; % convert diameter to radius
...
for k = 1:3
    ...
    [coverageVecTemp, tripleCoverageVecTemp]     = mainCoverage(tspan,  sample, dirSun, R, ...
                    ConstellationParameters, MapParameters, VisibilityParams, Constants);
...
hold on
for k = 1:3
    plot(tspan/T, coverageVec(:, k)*100, "LineWidth", 2)
end
...
hold on
for k = 1:3
    plot(tspan/T, tripleCoverageVec(:, k)*100, "LineWidth", 2)
end
lgd = legend(legendConstellationType);
...
function [coverageVec, tripleCoverageVec]     = mainCoverage(tspan,  sample, dirSun, R, ...
                ConstellationParameters, MapParameters, VisibilityParameters, Constants)
    ...
    rObsMat = convertStructToMatAtTime(Constellation, MapParameters.timestepMap);
    
    % 3) Compute coverage
    
    % Compute visibility map
    [rTrgMat, raDecTrgMat] = initialiseTargetPoints(MapParameters.sizeMap, MapParameters.altitudeMap, Constants);
    visibilityCountVec = countVisibilityMatrix(rTrgMat, rObsMat, dirSun, R, MapParameters.trgSizeMap, VisibilityParameters);
    [raMat, decMat, visibilityCountMat] = convertVecToMat(raDecTrgMat, visibilityCountVec, MapParameters.sizeMap);
    disp("Visibility map done, time: "+toc)
    
    % Compute coverage at all timesteps
    coverageVec = computeCoverage(Constellation, 1, sample, dirSun, R, VisibilityParameters);
    disp("Single coverage done, time: "+toc)
    tripleCoverageVec = computeCoverage(Constellation, 3, sample, dirSun, R, VisibilityParameters);
    disp("Triple coverage done, time: "+toc)

    % 1) Coverage map
    ConstellationOrbits = propagateOrbits(size(tspan, 2), ConstellationParameters, Constants); % propagate orbits only for plotting
    figure()
    m = mesh(rad2deg(raMat), rad2deg(decMat), visibilityCountMat);
    hold on
    for iOrbits = 1:ConstellationParameters.nOrb
        plotRaDec3D(ConstellationOrbits(iOrbits).raDec, "red", 1, 1e5)
    end
    m.FaceColor = 'flat';
    view(2)
    axis equal
...
function magnitude = computeApparentMagnitude(rObs, rTrg, dirSun, trgSize, rho, F_diff, F_spec, beta, Constants)
    ...
    %   - trgSize   : size of target in m (diameter)
